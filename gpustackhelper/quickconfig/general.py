...
class GeneralConfigPage(DataBindWidget):
    ...
    def _get_role_group(self, selection_layout: QLayout) ->QGroupBox:
        ...
        for _, (attr, title) in enumerate((('server_url', 'Server URL:'), ('token','Token:')), start=1):
            label, input = fixed_titled_input(title)
            self.config_binders.append(CleanConfig.bind(attr, input, ignore_zero_value=True))
    ...
    def __init__(self, cfg: HelperConfig, onShowSignal: SignalInstance, onSaveSignal: SignalInstance):
